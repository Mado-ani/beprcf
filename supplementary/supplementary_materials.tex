...
All chains show excellent mixing with no evidence of multimodality.\\
\textit{Generated from \texttt{R/03\_figures\_tables.R}}}}
\caption{Trace plots for key BEPRCF parameters from four NUTS chains.}
\label{fig:trace}
\end{figure}

Figure~\ref{fig:trace} displays trace plots for key model parameters. All four chains show excellent mixing with no evidence of multimodality or poor exploration. The $\widehat{R}$ statistic was below 1.005 for all monitored parameters, as reported in Section~5.2 of the manuscript.
...
indicate proper chain mixing across all four chains.\\
\textit{Generated from \texttt{R/03\_figures\_tables.R}}}}
\caption{Rank histograms for key BEPRCF parameters.}
\label{fig:rank}
\end{figure}

Figure~\ref{fig:rank} displays rank histograms for the key parameters, providing a complementary convergence assessment that is sensitive to chain misbehavior not captured by $\widehat{R}$ alone (Vehtari et al., 2021).
...
\newblock The Extended Odd Weibull--Power Lindley distribution with 
application to intraocular pressure data.
...
\newblock The Ocular Hypertension Treatment Study: baseline factors that 
...
Vehtari~A, Gelman~A, Simpson~D, Carpenter~B, and B\"urkner~P-C.
