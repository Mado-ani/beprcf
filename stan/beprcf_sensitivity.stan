// BEPRCF Sensitivity Analysis
// Includes shared cure frailty to test conditional independence assumption
...
functions {
  real exowpolo_cdf(real y, real a, real b, real alpha, real beta, real eta) {
    ...
    return fmin(1.0, fmax(0.0, 1.0 - inner^(-1.0 / b)));
  }
...
parameters {
  ...
  vector[P] tau_beta;
  vector[P] tau_pi;
  vector[P] tau_delta_11;
  vector[P] tau_delta_10;
  vector[P] tau_delta_01;
...
transformed parameters {
  ...
  for (i in 1:N) {
    vector[4] linpred;
    linpred[1] = X[i] * delta_11 + u[i];
    linpred[2] = X[i] * delta_10 + u[i];
    linpred[3] = X[i] * delta_01 + u[i];
    linpred[4] = 0;
    pi_rs[i] = softmax(linpred)';
  }
}
...
generated quantities {
  ...
  real cure_frailty_var = sigma_u^2;
  vector[N] log_lik;
  for (i in 1:N) {
